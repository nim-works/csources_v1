...
N_LIB_PRIVATE N_NIMCALL(NIM_BOOL, modeIsDir_systemZio_377)(mode_t m);
...
{	result = (NIM_BOOL)0;
	p = fopen(nimToCStringConv(filename), FormatOpen_systemZio_371[(mode)- 0]);
	{
		...
		f2 = ((FILE*) (p));
		{
			...
			T7_ = (((NI32) 0) <= T9_);
			if (!(T7_)) goto LA10_;
			T7_ = modeIsDir_systemZio_377(res.st_mode);
			LA10_: ;
			if (!T7_) goto LA11_;
...
N_LIB_PRIVATE N_NIMCALL(FILE*, open_systemZio_433)(NimStringDesc* filename, tyEnum_FileMode__ZJfK20XeZ9bv2j1pZjw9aswg mode, NI bufSize) {
	...
	result = (FILE*)0;
	{
		...
		NimStringDesc* T7_;
		T3_ = (NIM_BOOL)0;
		T3_ = open_systemZio_392(&result, filename, mode, bufSize);
		if (!!(T3_)) goto LA4_;
