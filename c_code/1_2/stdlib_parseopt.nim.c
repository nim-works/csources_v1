...
N_LIB_PRIVATE N_NIMCALL(void, handleShortOption_pureZparseopt_127)(tyObject_OptParser__l9cyoK9aLLTUH7LBRIbLqETg* p, NimStringDesc* cmd) {
	...
	}
	{
		...
		unsureAsgnRef((void**) (&(*p).val), substr_system_7821(cmd, i));
		(*p).pos = ((NI) 0);
		(*p).idx += ((NI) 1);
	}
	goto LA9_;
		...
		i += ((NI) 1);
		{
			...
			}
			{
				...
				LA57_: ;
				{
					...
					unsureAsgnRef((void**) (&(*p).val), substr_system_7821((*p).cmds->data[(*p).idx], i));
				}
				LA62_: ;
			}
			goto LA40_;
...
N_LIB_PRIVATE N_NIMCALL(NimStringDesc*, npocmdLineRest)(tyObject_OptParser__l9cyoK9aLLTUH7LBRIbLqETg* p) {
	...
	result = quoteShellCommand_pureZos_733(T2_->data, (T2_ ? T2_->Sup.len : 0));
	return result;
}
