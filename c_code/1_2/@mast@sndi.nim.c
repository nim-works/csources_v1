...
{nimErr_ = nimErrorFlag();
	T1_ = (NIM_BOOL)0;
	T1_ = isEmpty__frontZoptions_7838(filename);
...
{nimErr_ = nimErrorFlag();
	...
	write__systemZio_281(f, TM__QVKrxgdkvZgAHGFrhgVOcA_8);
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	}BeforeRet_: ;
}
N_LIB_PRIVATE N_NIMCALL(void, doWrite__astZndi_12)(tyObject_NdiFile__9cEuXfAnHp9bnWFK409cMtsEw* f, tyObject_TSym__RjHO5eqpy8QqcomFydmCag* s, tyObject_ConfigRefcolonObjectType___LaK09ayL9alwKUad7rts0rTw* conf) {
...
appendString((&(*f).buf), TM__QVKrxgdkvZgAHGFrhgVOcA_5);
	...
	write__systemZio_470((*f).f, T2_, 2);
	if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
...
{nimErr_ = nimErrorFlag();
	{
		...
		T5_ = (FILE*)0;
		T5_ = open__systemZio_593((*f).filename, ((tyEnum_FileMode__ZJfK20XeZ9bv2j1pZjw9aswg) 1), ((NI) 8000));
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
		(*f).f = T5_;
		{
			NimStringV2 colontmp_;
...
appendString((&T11_), (*f).filename);
		...
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
		reset__frontZoptions_5274((&(*f).filename));
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	}
	LA3_: ;
	}BeforeRet_: ;
