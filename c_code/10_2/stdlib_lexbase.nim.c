...
N_LIB_PRIVATE N_NIMCALL(void, fillBuffer_pureZlexbase_16)(tyObject_BaseLexer__MAw8rJZtJ279bVV3TWMtXSg* L);
...
N_LIB_PRIVATE N_NIMCALL(void, skipUtf8Bom_pureZlexbase_112)(tyObject_BaseLexer__MAw8rJZtJ279bVV3TWMtXSg* L);
N_LIB_PRIVATE N_NIMCALL(NI, fillBaseLexer_pureZlexbase_68)(tyObject_BaseLexer__MAw8rJZtJ279bVV3TWMtXSg* L, NI pos);
...
N_LIB_PRIVATE N_NIMCALL(void, fillBuffer_pureZlexbase_16)(tyObject_BaseLexer__MAw8rJZtJ279bVV3TWMtXSg* L) {
	...
	toCopy = (NI)(((*L).buf ? (*L).buf->Sup.len : 0) - (NI)((*L).sentinel + ((NI) 1)));
	{
		...
		moveMem_system_1731(((void*) ((&(*L).buf->data[((NI) 0)]))), ((void*) ((&(*L).buf->data[(NI)((*L).sentinel + ((NI) 1))]))), ((NI) (toCopy)));
	}
	LA3_: ;
	T5_ = dotdot__pureZunicode_840(toCopy, (NI)((NI)((NI)(toCopy + (*L).sentinel) + ((NI) 1)) - ((NI) 1)));
	charsRead = readDataStr_pureZstreams_81((*L).input, (&(*L).buf), T5_);
		...
		s -= ((NI) 1);
		{
			while (1) {
				...
				LA19_: ;
				{
					...
					T22_ = dotdot__pureZunicode_840(oldBufLen, (NI)(((*L).buf ? (*L).buf->Sup.len : 0) - ((NI) 1)));
					charsRead = readDataStr_pureZstreams_81((*L).input, (&(*L).buf), T22_);
					{
...
N_LIB_PRIVATE N_NIMCALL(void, open_pureZlexbase_122)(tyObject_BaseLexer__MAw8rJZtJ279bVV3TWMtXSg* L, tyObject_StreamObj__THYguAi9bSgidczZ3ywEIMQ* input, NI bufLen, tySet_tyChar__nmiMWKVIe46vacnhAFrQvw refillChars) {
	...
	(*L).lineStart = ((NI) 0);
	(*L).lineNumber = ((NI) 1);
	fillBuffer_pureZlexbase_16(L);
	skipUtf8Bom_pureZlexbase_112(L);
}
N_LIB_PRIVATE N_NIMCALL(NI, fillBaseLexer_pureZlexbase_68)(tyObject_BaseLexer__MAw8rJZtJ279bVV3TWMtXSg* L, NI pos) {
	NI result;
	result = (NI)0;
...
N_LIB_PRIVATE N_NIMCALL(NI, handleCR_pureZlexbase_79)(tyObject_BaseLexer__MAw8rJZtJ279bVV3TWMtXSg* L, NI pos) {
	...
	result = (NI)0;
	(*L).lineNumber += ((NI) 1);
	result = fillBaseLexer_pureZlexbase_68(L, pos);
	{
		if (!((NU8)((*L).buf->data[result]) == (NU8)(10))) goto LA3_;
		result = fillBaseLexer_pureZlexbase_68(L, result);
	}
	LA3_: ;
	(*L).lineStart = result;
	return result;
