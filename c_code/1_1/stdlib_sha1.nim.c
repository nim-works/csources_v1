...
N_LIB_PRIVATE N_NIMCALL(NIM_BOOL, eqeq___stdZsha49_945)(tyArray__9bMdWY3rbF7YgboQDvW9b15Q x, tyArray__9bMdWY3rbF7YgboQDvW9b15Q y);
...
{nimErr_ = nimErrorFlag();
	...
	f = open__systemZio_593(filename, ((tyEnum_FileMode__ZJfK20XeZ9bv2j1pZjw9aswg) 0), ((NI) -1));
	...
	buffer = mnewString(((NI) 8192));
	{
		while (1) {
			...
			length = readChars__systemZio_202(f, buffer.p->data, buffer.len);
			if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
			{
...
{nimErr_ = nimErrorFlag();
	result = (NIM_BOOL)0;
	result = eqeq___stdZsha49_945(a, b);
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	}BeforeRet_: ;
	return result;
}
